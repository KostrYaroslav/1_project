...
static bool s_searching_open = false;

// Статистика для вывода на сайт
void wifi_get_current_stats(char *ssid, int *rssi) {
    wifi_ap_record_t ap_info;
    if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
        strncpy(ssid, (char *)ap_info.ssid, 32);
        *rssi = ap_info.rssi;
    } else {
        strcpy(ssid, "Disconnected");
        *rssi = 0;
    }
}
...
static void connect_to_open_network(void) {
    ...
    for (int i = 0; i < ap_count; i++) {
        if (ap_info[i].authmode == WIFI_AUTH_OPEN) {
            ESP_LOGI(TAG, "Найдена открытая сеть: %s", ap_info[i].ssid);
...
static void wifi_event_handler(void* arg, esp_event_base_t base, int32_t id, void* data) {
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < 5) {
            esp_wifi_connect();
            s_retry_num++;
            ESP_LOGI(TAG, "Попытка %d...", s_retry_num);
